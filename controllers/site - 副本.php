...
class Site extends IController
{
	...
	function index()
	{
		...
		//获取用户基本信息
		$this->memberRow = Api::run('getMemberInfo',$this->user['user_id']);
		if(!isset($this->memberRow['area']) || empty($this->memberRow['area']))
		{
			$this->redirect('/ucenter/info');exit();
		}
		$this->marketList=self::getMarket($this->memberRow['area']);
		$this->index_slide = Api::run('getBannerList');
		$this->redirect('index');
	}
	function getMarket($area)
	{
		$area=(int)$area;
		$marketDB  = new IModel('market');
		$marketData = $marketDB->query("district = ".$area);
		return $marketData;
	}
	function search()
	{
