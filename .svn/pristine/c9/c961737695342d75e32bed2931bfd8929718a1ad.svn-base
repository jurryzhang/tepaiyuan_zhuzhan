	...
	public function goodsCount($buyInfo,$promo='',$active_id='')
	{
    	...
    	else
    	{
	    	...
	    	if(isset($buyInfo['goods']['id']) && $buyInfo['goods']['id'])
	    	{
	    		...
	    		$goodsObj->join ='LEFT JOIN seller as s on s.id=go.seller_id';
	    		$goodsObj->where  = 'go.id in ('.$goodsIdStr.')';
	    		$goodsObj->fields = 'go.name,go.cost_price,go.id as goods_id,go.img,go.sell_price,go.point,go.weight,go.store_nums,go.exp,go.goods_no,0 as product_id,go.seller_id,s.true_name as sname';
	    	...
	    	if(isset($buyInfo['product']['id']) && $buyInfo['product']['id'])
	    	{
	    		...
	    		$productObj->join ='LEFT JOIN seller as s on s.id=go.seller_id';
	    		$productObj->where  = 'pro.id in ('.$productIdStr.') and go.id = pro.goods_id';
	    		$productObj->fields = 'pro.sell_price,pro.cost_price,pro.weight,pro.id as product_id,pro.spec_array,pro.goods_id,pro.store_nums,pro.products_no as goods_no,go.name,go.point,go.exp,go.img,go.seller_id,s.true_name as sname';
