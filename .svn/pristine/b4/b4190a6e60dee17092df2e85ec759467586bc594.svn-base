...
class Payment
{
	 ...
	 * @return array 支付提交信息
	 */
	public static function getPaymentInfo2($payment_id,$type,$argument)
	{
		//最终返回值
		$payment = array();
		...
		if($type == 'order')
		{
			...
			foreach($orderIdArray as $key => $order_id)
			{
				...
				if($key==0)
				{
					...
					$omData = $omObj->find();
					// echo $omObj->getSql();
					// echo '<pre>';var_dump($omData);
					$yunfei=$omData[0]['yunfei'];
				}
				// echo $key.'----'.$yunfei.'<br>';

				// echo $orderRow['order_amount'].'<br>';
				$order_amount=$orderRow['order_amount']+$yunfei;

				$M_Amount   += $order_amount;
				$M_OrderNO[] = $orderRow['order_no'];
			}
			// echo $M_Amount.'<br>';
