...
class Block extends IController
{
    ...
    */
	public function doPay()
	{
		...
		// echo '<pre>';var_dump($paymentInstance);exit();
		...
		{
			$sendData = $paymentInstance->getSendData(Payment::getPaymentInfo2($payment_id,'order',$order_id));
			// echo '<pre>';var_dump($sendData);exit();
    ...
    */
    ...
    //通过市场id字串获取 当前地址信息 获取运费 以及距离
    public function getyunfei()
    {
    	$marketid_str = IFilter::act(IReq::get('marketid_str'));
    	$cityVal = IFilter::act(IReq::get('cityVal'));
    	$addVal = IFilter::act(IReq::get('addVal'));
    	$weightVal = IFilter::act(IReq::get('weightVal'),'float');
    	// 先获取当前地址经纬度
    	$lnglatData=area::getTraceByName($cityVal,$addVal);
    	$marketObj = new IQuery('market');
    	$marketObj->where='id in ('.$marketid_str.') ';
    	$marketObj->fields='id,lat,lng';
    	$market=$marketObj->find();
    	$res=array();
    	foreach ($market as $k => $v) {
    		$lat=$v['lat'];
    		$lng=$v['lng'];
    		$juliData=area::getDistance($lat,$lng,$lnglatData['lat'],$lnglatData['lng']);
    		$res[$v['id']]['juli']=$juliData['text'];
    		$res[$v['id']]['juli_val']=$juliData['value'];
            $yunfei=(($juliData['value']/1000)*$this->_siteConfig->freight_rate_value);
            $res[$v['id']]['yunfei']=round(round($yunfei,2)*$weightVal,2);
    	}
    	die(JSON::encode($res));
    }
