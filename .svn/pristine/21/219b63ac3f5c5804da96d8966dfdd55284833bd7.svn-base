...
class IModel
{
	...
	/**
	 * [多条数据更新 不可随意执行 当前限制类型为字符]
	 * @param  [array] $data [description]
	 * @param  [str] $set  [description]
	 * @param  [str] $case [description]
	 * @return [type]       [description]
	 */
	public function updateSetCase($data,$set,$case)
	{
		if(!is_array($data) || empty($data) || empty($set) || empty($case)){return '当前方法不能随意使用';}
		$name='\'';
		$name .= implode('\',\'', array_keys($data));
		$name.='\'';

		$sql = "UPDATE ".$this->tableName." SET ".$set." = CASE  ".$case." ";
		foreach ($data as $n => $v) {
		    $sql .= sprintf("WHEN '%s' THEN '%s' ", $n, $v);
		}
		$sql .= "END WHERE name IN ($name)";
		echo $sql.'<br>';
		return $this->db->query($sql);
	}
