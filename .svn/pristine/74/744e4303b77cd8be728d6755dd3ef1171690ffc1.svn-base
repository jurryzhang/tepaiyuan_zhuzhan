...
class Cart extends IInterceptorBase
{
	...
	*/
	...
	public function add($gid, $num = 1 ,$type = 'goods')
	{
		...
		if(isset($cartInfo[$type][$gid]))
		{
			$resNum=$cartInfo[$type][$gid]+($num);
			if($resNum<=0)
			{
				$this->error = '购买的数量必须大于1件';
				return false;
			}
		}
	...
	public function del2($gstr='',$pstr='')
	{
		$cartInfo = $this->getMyCartStruct();
		if($gstr!='')
		{
			$data=explode(",",trim($gstr,','));
			
			$type='goods';
			foreach ($data as $k => $v) {
				//删除商品数据
				if(isset($cartInfo[$type][$v]))
				{
					unset($cartInfo[$type][$v]);
					$this->setMyCart($cartInfo);
				}
				else
				{
		...
		if($pstr!='')
		{
			$pstrD=explode(",",trim($pstr,','));
			
			$type='product';
			foreach ($pstrD as $k => $v) {
				//删除商品数据
				if(isset($cartInfo[$type][$v]))
				{
					unset($cartInfo[$type][$v]);
