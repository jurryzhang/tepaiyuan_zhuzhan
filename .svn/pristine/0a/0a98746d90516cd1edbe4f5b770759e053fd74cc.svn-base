...
class Order extends IController implements adminAuthorization
{
     ...
     * @brief 订单列表
     */
    public function ordermarket_list()
    {
		//搜索条件
		$search = IReq::get('search');
		$page   = IReq::get('page') ? IFilter::act(IReq::get('page'),'int') : 1;
		// //条件筛选处理
		list($join,$where) = order_class::getOMSearchCondition($search);
		//拼接sql
		$orderHandle = new IQuery('order_market as om');
		$orderHandle->order  = "om.id desc";
		$orderHandle->fields = "om.*,m.name as m_name";
		$orderHandle->page   = $page;
		$orderHandle->where  = $where;
		$orderHandle->join   = $join;
		$orderHandle->getSql();
		...
		$this->redirect("ordermarket_list");
    }
    /**
     * @brief 订单删除功能_删除到回收站
     */
	 ...
	 * @brief查看退款单
	 */
	public function returnment_show()
	{
	 	//获得post传来的退款单id值
	 	$returnment_id = IFilter::act(IReq::get('id'),'int');
	 	$data = array();
	 	if($returnment_id)
	 	{
	 		$tb_returnment = new IQuery('returnment_doc as c');
	 		$tb_returnment->join=' left join order as o on c.order_id=o.id left join user as u on u.id = c.user_id';
	 		$tb_returnment->fields = 'o.order_no,o.create_time,u.username,c.*';
	 		$tb_returnment->where = 'c.id='.$returnment_id;
	 		$returnment_info = $tb_returnment->find();
	 		if($returnment_info)
	 		{
	 			$data = current($returnment_info);
	 			$this->setRenderData($data);
	 			$this->redirect('returnment_show',false);
	 		}
	 	}

	 	if(!$data)
		{
			$this->redirect('order_returnment_list');
		}
	}
	/**
	 * @brief查看申请退款单
	 */
	public function returnment_doc_show()
	{
	 	//获得post传来的申请退款单id值
	 	$returnment_id = IFilter::act(IReq::get('id'),'int');
	 	if($returnment_id)
	 	{
	 		$returnsDB = new IModel('returnment_doc');
	 		$data = $returnsDB->getObj('id = '.$returnment_id);
	 		if($data)
	 		{
	 			$this->setRenderData($data);
	 			$this->redirect('returnment_doc_show',false);
	 			return;
	 		}
	 	}

	 	$this->redirect('returnment_list');
	}
	//删除申请退款单
	public function returnment_doc_del()
	{
		//获得post传来的申请退款单id值
		$returnment_id = IFilter::act(IReq::get('id'),'int');
		if(is_array($returnment_id))
		{
			$returnment_id = implode(",",$returnment_id);
		}
		if($returnment_id)
		{
			$tb_returnment_doc = new IModel('returnment_doc');
			$tb_returnment_doc->setData(array('if_del' => 1));
			$tb_returnment_doc->update("id IN ($returnment_id)");
		}

		$logObj = new log('db');
		$logObj->write('operation',array("管理员:".ISafe::get('admin_name'),"退款单移除到回收站",'移除的ID：'.$returnment_id));
	 ...
	 * @brief更新申请退款单
	 */
	public function returnment_doc_show_save()
	{
		//获得post传来的退款单id值
		$returnment_id = IFilter::act(IReq::get('id'),'int');
		$pay_status = IFilter::act(IReq::get('pay_status'),'int');
		...
		//获得returnment_doc对象
		$tb_returnment_doc = new IModel('returnment_doc');
		$tb_returnment_doc->setData(array(
			'pay_status'   => $pay_status,
			'dispose_idea' => $dispose_idea,
			'dispose_time' => ITime::getDateTime(),
			'admin_id'     => $this->admin['admin_id'],
		...
		if($returnment_id)
		{
			$tb_returnment_doc->update('id='.$returnment_id);

			$logObj = new log('db');
			$logObj->write('operation',array("管理员:".ISafe::get('admin_name'),"修改了退款单",'修改的ID：'.$returnment_id));
		}
		$this->redirect('returnment_list');
	}
	/**
	 * @brief 退款单页面
	 */
	public function order_returnment()
	{
		//去掉左侧菜单和上部导航
		$this->layout='';
		$orderId   = IFilter::act(IReq::get('id'),'int');
		$returnsId = IFilter::act(IReq::get('returns_id'),'int');
		...
		if($orderId)
		{
			...
			{
				$returnsDB  = new IModel('returnment_doc');
				$returnsRow = $returnsDB->getObj('id = '.$returnsId);
				$data['returns'] = $returnsRow;
			}
			$this->setRenderData($data);
			$this->data = $data;
			$this->redirect('order_returnment');
			return;
		}
		die('订单数据不存在');
	}
	 ...
	 * @brief 保存退款单页面
	 */
	public function order_returnment_doc()
	{
		$returns_id = IFilter::act(IReq::get('returns_id'),'int');
		$amount   = IFilter::act(IReq::get('amount'),'float');
		$order_id = IFilter::act(IReq::get('id'),'int');
		$order_no = IFilter::act(IReq::get('order_no'));
		$user_id  = IFilter::act(IReq::get('user_id'),'int');
		...
		{
			$updateData = array('amount' => $amount);
			$tb_returnment_doc->setData($updateData);
			$tb_returnment_doc->update("id = ".$returns_id);
		}
		//2,无退款申请单，必须生成退款单
		else
		{
			...
			$tb_returnment_doc->setData($updateData);
			$returns_id = $tb_returnment_doc->add();
		}

		$result = Order_Class::refund($returns_id,$this->admin['admin_id'],'admin',$way);
		if(is_string($result))
		{
			$tb_returnment_doc->rollback();
			die('<script text="text/javascript">parent.actionCallback("'.$result.'");</script>');
		}
		else
		{
	 ...
	 * @brief 退款单删除功能_删除到回收站
	 */
    public function returnment_del()
    {
    	//post数据
    	$id = IFilter::act(IReq::get('id'),'int');
    	//生成order对象
    	$tb_order = new IModel('returnment_doc');
    	$tb_order->setData(array('if_del'=>1));
    	if(!empty($id))
		{
			$tb_order->update(Util::joinStr($id));
		...
		{
			$this->redirect('order_returnment_list',false);
			Util::showMessage('请选择要删除的数据');
		}
    }
    /**
	 * @brief 退款单删除功能_删除回收站中的数据，彻底删除
	 */
    public function returnment_recycle_del()
    {
    	//post数据
    	$id = IFilter::act(IReq::get('id'),'int');
    	//生成order对象
    	...
    	if(!empty($id))
		{
			...
			$this->redirect('returnment_recycle_list');
		}
		else
		{
			$this->redirect('returnment_recycle_list',false);
			Util::showMessage('请选择要删除的数据');
		}
    }
    /**
	 * @brief 还原还款单列表
	 */
    public function returnment_recycle_restore()
    {
    	//post数据
    	$id = IFilter::act(IReq::get('id'),'int');
    	//生成order对象
