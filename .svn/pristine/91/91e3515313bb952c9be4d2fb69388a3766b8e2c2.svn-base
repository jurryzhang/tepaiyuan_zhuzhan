	...
	public function goodsCount($buyInfo,$promo='',$active_id='')
	{
    	...
    	if($promo && $active_id)
    	{
	    	...
	    	if($activeResult === true)
	    	{
    			...
    			if(!isset($this->market[$typeRow['market_id']]))
    			{
    				$this->market[$typeRow['market_id']] = 0;
    			}
    			$this->market[$typeRow['market_id']] += $typeRow['sum'];
    	...
    	else
    	{
			...
			//获取商品或货品数据
			/*Goods 拼装商品优惠价的数据*/
			// echo '<pre>';var_dump($buyInfo);exit();
	    	if(isset($buyInfo['goods']['id']) && $buyInfo['goods']['id'])
	    	{
	    		//购物车中的商品数据
	    		$goodsIdStr = join(',',$buyInfo['goods']['id']);
	    		$goodsObj   = new IQuery('goods as go');
	    		$goodsObj->join ='LEFT JOIN seller as s on s.id=go.seller_id LEFT JOIN market  as m on s.market_id=m.id';
	    		$goodsObj->where  = 'go.id in ('.$goodsIdStr.')';
	    		$goodsObj->fields = 'go.name,go.cost_price,go.id as goods_id,go.img,go.img_pathtype,go.sell_price,go.point,go.weight,go.store_nums,go.exp,go.goods_no,0 as product_id,go.seller_id,s.true_name as sname,m.id as market_id,m.name as mnam';


	    		$goodsList  = $goodsObj->find();
	    		// echo  $goodsObj->getSql();
	    		// echo '5655';exit();
	    		...
	    		{
	    			//检查库存
	    			// echo $buyInfo['goods']['data'][$val['goods_id']]['count'].'----'.$buyInfo['goods']['data'][$val['goods_id']]['count'].'------'.$val['store_nums'].'<br>';
	    			...
	    			{
	    				$val['market_id']=0;
	    				$goodsList[$key]['market_id']=0;
	    			}

	    			if(!isset($this->market[$val['market_id']]))
	    			{
	    				$this->market[$val['market_id']] = 0;
	    			}
	    			$this->market[$val['market_id']] += $goodsList[$key]['sum'];
	    	...
	    	if(isset($buyInfo['product']['id']) && $buyInfo['product']['id'])
	    	{
	    		...
	    		$productObj->join ='LEFT JOIN seller as s on s.id=go.seller_id LEFT JOIN market  as m on s.market_id=m.id';
	    		$productObj->where  = 'pro.id in ('.$productIdStr.') and go.id = pro.goods_id';
	    		$productObj->fields = 'pro.sell_price,pro.cost_price,pro.weight,pro.id as product_id,pro.spec_array,pro.goods_id,pro.store_nums,pro.products_no as goods_no,go.name,go.point,go.exp,go.img,go.img_pathtype,go.seller_id,s.true_name as sname,m.id as market_id,m.name as mname';

	    		$productList  = $productObj->find();
	    		// echo $productObj->getSql();exit();
	    		//开始优惠情况判断
	    		foreach($productList as $key => $val)
	    		{
	    			...
	    			{
	    				$val['market_id']=0;
	    				$productList[$key]['market_id']=0;
	    			}
	    			if(!isset($this->market[$val['market_id']]))
	    			{
	    				$this->market[$val['market_id']] = 0;
	    			}
	    			$this->market[$val['market_id']] += $productList[$key]['sum'];
	    	...
	    	if($user_id)
	    	{
	    		...
	    		foreach($this->seller as $seller_id => $sum)
	    		{
			    	...
			    	// echo '<pre>';var_dump($proObj->getInfo());
