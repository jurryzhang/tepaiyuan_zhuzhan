...
class search_goods
{
	...
	public static function find($defaultWhere = '',$limit = 21,$isCondition = true)
	{
		...
		$goodsObj->fields   = 'go.id,go.name,go.sell_price,go.market_price,go.store_nums,go.img,go.sale,go.grade,go.comments,go.favorite,go.img_pathtype';
		...
		//(5),商品品牌
		$where[] = intval(IReq::get('brand')) ? 'go.brand_id = '.intval(IReq::get('brand')) : '';
