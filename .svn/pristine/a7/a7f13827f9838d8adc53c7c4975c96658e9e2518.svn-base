...
class Order_Class
{
	...
	public static function updateOrderStatus($orderNo,$admin_id = '',$note = '')
	{
		...
		else if($orderRow['pay_status'] == 0)
		{
			...
			$omObj  = new IModel('order_market');
			$omObj->setData(array('pay_status' => 1));
			$is_success = $omObj->update('market_id = "'.$market_id.'" and uid="'.$uid.'" and pay_status=0 and find_in_set(\''.$order_id.'\',orderid_str)');
