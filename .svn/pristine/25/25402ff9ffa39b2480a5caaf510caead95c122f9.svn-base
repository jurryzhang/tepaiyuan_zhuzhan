	...
	function save_conf()
	{
		...
		switch($form_index)
		{
			...
			case "other_conf":
			{
				...
				if(isset($_POST['baidu_map_key']) && $_POST['baidu_map_key']=="")
				{
					$_POST['baidu_map_key']="g0OPrhSYBqORW9q4QdlomzUtqPYbbfG0";
				}
				if(isset($_POST['freight_rate_value']) && $_POST['freight_rate_value']=="")
				{
					$_POST['freight_rate_value']="1";
				}
			}
			break;
		...
		else
		{
			...
			//将部分数据更新到配置数据库中
			$configDB=new IModel('config');
			$configData=array();

			if($inputArray['baidu_map_key']!=''){$configData['baidu_map_key']=$inputArray['baidu_map_key'];}
			if($inputArray['freight_rate_value']!=''){$configData['freight_rate_value']=$inputArray['freight_rate_value'];}
			if($inputArray['goods_no_pre']!=''){$configData['goods_no_pre']=$inputArray['goods_no_pre'];}
			if($inputArray['store_num_warning']!=''){$configData['store_num_warning']=$inputArray['store_num_warning'];}
			
			if($configData){

				$configDB->updateSetCase($configData,'value','name');
			}
