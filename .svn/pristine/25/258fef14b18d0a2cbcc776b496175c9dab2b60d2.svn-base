...
class ProRule
{
	...
	public function getInfo()
	{
		...
		{
			$explain[$val['seller_id']][$key]['id'] = $val['seller_id'];
			$explain[$val['seller_id']][$key]['plan'] = $hostTag.$val['name'];
			$explain[$val['seller_id']][$key]['info'] = $this->typeExplain($val['award_type'],$val['condition'],$val['award_value']);
		}
		return $explain;
	}
	...
	private function satisfyPromotion($award_type = null)
	{
		...
		// $where    = ' type = 0 and is_close = 0 and start_time <= "'.$datetime.'" and end_time >= "'.$datetime.'" and seller_id = '.$this->seller_id;
