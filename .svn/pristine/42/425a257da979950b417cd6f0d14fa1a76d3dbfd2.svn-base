...
class Site extends IController
{
	...
	function index()
	{
		...
		{
			$this->redirect('/simple/login');exit();
		}
		// ICookie::set('curMarket','');
		$curMarket=ICookie::get('curMarket');
		// echo '<pre>';var_dump($curMarket);
		if($curMarket)
		{
			$this->getnum=$curMarket['getnum'];
	...
	function selectMarket()
	{
		...
		$curMarket=ICookie::get('curMarket');
		$curMarket['marketid']=$marketid;
		ICookie::set('curMarket',$curMarket);
		$result = array('code' => 1);
	...
	function getMarket()
	{
		...
		$district = IFilter::act(IReq::get('district'));
		$street = IFilter::act(IReq::get('street'));
		...
		//获取经纬度
		$latlng=area::getTraceByName($city,$district.$street);

		//通过名称获取id
		$areaData  = area::name_getid($data);
		// echo '<pre>';var_dump($areaData);
		$provinceid=$areaData['cookie']['provinceid']['id'];
		$cityid=$areaData['cookie']['city']['id'];
		$districtid=$areaData['cookie']['district']['id'];
		$streetid=$areaData['cookie']['street']['id'];

		//获取市场
		$wname=$areaData['cookie']['market']['name'];
		$wid=$areaData['cookie']['market']['id'];
		$where=$wname.'='.$wid.' and status=1';

		$marketDB  = new IModel('market');
		$marketData = $marketDB->query($where,'id,name');
		$sql=$marketDB->getSql();
		if(empty($marketData))
		{
			ICookie::set('curMarket',array('getnum'=>1,'dingwei'=>$district,'marketid'=>0,'latlng'=>$latlng));
			$result = array('code' => 0,'msg' => '您当前所在的城市暂无市场存在','marketid'=>0);
			die(JSON::encode($result));
		}else{
			//对市场进行select 拼接 设置当前市场  并获取数据
			$curData=self::getMaketTemp($marketData);
			$curData['dingwei']=$district;
			$curData['getnum']=1;
			$curData['latlng']=$latlng;

			ICookie::set('curMarket',$curData);
			$result = array('code' => 1,'msg' => '定位成功','marketTemp'=>$curData['temp'],'dingwei'=>$district,'curid'=>$curData['marketid']);
			die(JSON::encode($result));
		}
	}

	// function index_goodslist($curMarketid=0)
	// {
	// 	$curid=IFilter::act(IReq::get('curMarketid'),int);
	// 	$curMarketid=isset($curid)?$curid:$curMarketid;
		
	// 	//获取根目录路径
	// 	$path = $this->getViewPath().$this->getId();
 //    	//保存 购物清单模板
	// 	$ifile_shop = new IFile($path.'/_index_goodslist.html','w');
	// 	$ifile_shop->write($this->curMarketid);
	// }

	function getMaketTemp($marketData)
	{
		...
		foreach ($marketData as $k => $v) 
		{
			...
			//区判断
			//街道判断
			if($i==0 && $v['id']==$district && $v['id']==$street)
			{
				//区 街道 通过
				$i++;
				$select='selected';
			...
			$temp .="<option value='".$v['id']."' ".$select." >".$v['name']."</option>";
		}
		if($i==0)
		{
	...
	function products()
	{
		...
		$tb_goods_photo->fields = 'p.id AS photo_id,p.img,p.type as photo_type ';
		...
		// echo $tb_goods_photo->getsql();exit();
	...
	function robuyon()
	{
		...
		if($id)
		{
			$this->regiment_list = Api::run('getPromotionRowById',array('#id#',$id));
		...
		else
		{
			...
			{
				$this->regiment_list = Api::run('getPromotionList');
			}else{
				$this->regiment_list = Api::run('getPromotionMList',array('#market_id#',$this->curMarketid));
			}
		...
		{
			IError::show('当前没有可以参加的限时抢购活动');
		}

		//往期团购
		// $this->ever_list = Api::run('getEverRegimentList');
		$this->redirect("robuyon");
	}
	...
	function home()
	{
		...
		$this->setRenderData(array('sellerRow' => $sellerRow,'seller_id' => $seller_id,'cateid'=>$cateid));
		$this->redirect('home');
	}
}
