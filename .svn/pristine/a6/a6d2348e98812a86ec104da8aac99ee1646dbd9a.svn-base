...
return array(
	...
	'getGoodsInfo' => array(
		'query' => array(
			...
			'fields' => 'go.name,go.id as goods_id,go.img,go.img_pathtype,go.sell_price,go.point,go.weight,go.store_nums,go.exp,go.goods_no,0 as product_id,go.seller_id,s.market_id',
			'type'   => 'row',
		)
	),
	//取货品数据
	'getProductInfo' => array(
		'query' => array(
			...
			'join' => 'left join seller as s on s.id = go.seller_id',
			'fields' => 'pro.sell_price,pro.weight,pro.id as product_id,pro.spec_array,pro.goods_id,pro.store_nums,pro.products_no as goods_no,go.name,go.point,go.exp,go.img,go.seller_id,s.market_id',
			'type'   => 'row',
		)
	),
	//取文章置顶列表
	...
	'getPromotionList'=> array(
		'query' => array(
			...
			'fields'=>'p.end_time,go.name as title,go.img as img,p.name as name,p.award_value as award_value,go.id as goods_id,p.id as p_id,end_time,go.unit,go.sell_price',
	...
	//限时抢购列表
	'getPromotionMList'=> array(
		'query' => array(
			'name' => 'promotion as p',
			'join' => 'left join goods as go on go.id = p.condition left join seller as s on go.seller_id=s.id left join market as m on s.market_id=m.id',
			'fields'=>'p.end_time,go.name as title,go.img as img,p.name as name,p.award_value as award_value,go.id as goods_id,p.id as p_id,end_time,go.unit,go.sell_price',
			'where'=>'m.id in(#market_id#) and p.type = 1 and p.is_close = 0 and go.is_del = 0 and NOW() between start_time and end_time AND go.id is not null',
			'order'=>'p_id desc',
			'limit'=>'10'
		)
	),
	...
	'getCategoryExtendList'=>array(
	    'query'=>array(
	    	...
	    	'order' => 'go.sort asc',
	    	'fields'=> 'go.id,go.name,go.img,go.sell_price,go.market_price,go.unit,go.img_pathtype',
	    )
	),
	'getCategoryMarketList'=>array(
	    'query'=>array(
	    	'name'  => 'category_extend as ca',
	    	'join'  => 'left join goods as go on go.id = ca.goods_id left join seller as s on go.seller_id=s.id left join market as m on s.market_id=m.id',
	    	'where' => 'ca.category_id in(#categroy_id#) and m.id in(#market_id#) and go.is_del = 0',
	    	'order' => 'go.sort asc',
	    	'fields'=> 'go.id,go.name,go.img,go.sell_price,go.market_price,go.unit,go.img_pathtype',
	    )
	),
	...
	'getCategoryExtendByCommendid'=>array(
	    'query'=>array(
	    	...
	    	'fields'=> 'DISTINCT go.id,go.img,go.img_pathtype,go.sell_price,go.name,go.market_price,go.description',
		   	'order' => 'go.sort asc',
	    	'limit' => 6,
	    )
	),
	...
	'getInOrderGoodsListByGoodsid'=>array(
	 	 'query'=>array(
	    	'name'  => 'order_goods as og',
	    	'join'  => 'left join goods as go on og.goods_id = go.id',
	    	'where' => 'order_id in(#orderid_str#)  ',
	    	'fields'=> 'og.*,go.point',
	    )
	),
	...
	//获取指定特派员
	'getFreigntInfo' => array(
		'query' => array(
			'name'   => 'freight',
			'where'  => 'id=#freight_id#',
			'fields' => 'id,plate_num,name,telphone,landline',
		)
	),
