...
class Simple extends IController
{
    ...
    function regAct()
    {
        ...
        if($_FILES['paper_img']['name']=='' || $_FILES['certif_img']['name']=='' || $_FILES['carinfo_img']['name']=='')
        {
            $this->redirect('reg',false);
    ...
    {
        $goodsStr  = IReq::get('delStr');

        $cartObj   = new Cart();
        $cartInfo  = $cartObj->getMyCart();
        $delResult = $cartObj->del2($goodsStr);
    ...
    function cart($redirect = false)
    {
        ...
        if($this->user['user_id'] == null)
        {
            ...
            // 获取当前市场的经纬度
            $userLatlng=ICookie::get('userLatlng');
        }
        ...
        foreach ($marketL as $k => $v) {
            ...
            $lat=$v['lat'];
            $lng=$v['lng'];
            $juliData=area::getDistance($lat,$lng,$userLatlng['lat'],$userLatlng['lng']);

            $mListLatlng[$v['id']]['juli']=$juliData['text'];
            $mListLatlng[$v['id']]['yunfei']=(($juliData['value']/1000)*$this->_siteConfig->freight_rate_value);
        }
        // echo '<pre>';var_dump($mListLatlng);
        ...
        foreach ($goodsList as $k => $v) {
            if(isset($marketArr[$v['market_id']]))
            {
                ...
                $marketArr[$v['market_id']]['juli']=$mListLatlng[$v['market_id']]['juli'];
                $marketArr[$v['market_id']]['yunfei']=$mListLatlng[$v['market_id']]['yunfei'];
            }else{
                $marketArr[$v['market_id']]['id']=$v['market_id'];
                $marketArr[$v['market_id']]['name']=$marketList[$v['market_id']];
                $marketArr[$v['market_id']]['num']=1;
    ...
    function cart2()
    {
        ...
        // $sellerList[0]='自营';
        // foreach ($sellerL as $k => $v) {
        //     $sellerList[$v['id']]=$v['true_name'];
        // }
        // $sellArr=array();
        // foreach ($goodsList as $k => $v) {
        //     if(isset($sellArr[$v['seller_id']]))
        //     {
        //         $sellArr[$v['seller_id']]['id']=$v['seller_id'];
        //         $sellArr[$v['seller_id']]['name']=$sellerList[$v['seller_id']];
        //         $sellArr[$v['seller_id']]['num']+=1;
        //         $sellArr[$v['seller_id']]['sum']+=$v['sum'];
        //     }else{
        //         $sellArr[$v['seller_id']]['id']=$v['seller_id'];
        //         $sellArr[$v['seller_id']]['name']=$sellerList[$v['seller_id']];
        //         $sellArr[$v['seller_id']]['num']=1;
