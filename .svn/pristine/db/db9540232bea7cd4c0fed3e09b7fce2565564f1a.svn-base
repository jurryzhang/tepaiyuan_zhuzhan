...
class Member extends IController implements adminAuthorization
{
	...
	function member_save()
	{
		...
		$person_charge         = IFilter::act(IReq::get('person_charge'));
		$tax_number         = IFilter::act(IReq::get('tax_number'));
	...
	public function seller_edit()
	{
		...
		$this->marketRow = Api::run('getMarketList');
	...
	{
		$market_id = IFilter::act(IReq::get('id'),'int');

		//修改页面
		if($market_id)
		{
			$sellerDB        = new IModel('market');
			$this->sellerRow = $sellerDB->getObj('id = '.$market_id);
		}
		$this->redirect('market_edit');
	}
	...
	public function seller_add()
	{
		...
		// echo '444444';
		$sellerRow['market_id'] = $market_id=IFilter::act(IReq::get('market_id'),'int');
		$marketDB = new IQuery('market');
		$marketDB->where = "id = ".$market_id;
		$marketData = $marketDB->find();
		$sellerRow['marketsplace'] = $marketData[0]['name'];
	...
	// 改变市场状态
	public function ajax_market_lock()
	{
		$id   = IFilter::act(IReq::get('id'));
		$lock = IFilter::act(IReq::get('lock'));
		$sellerObj = new IModel('market');
		$sellerObj->setData(array('status' => $lock));
		$sellerObj->update("id = ".$id);
		$sellerRow = $sellerObj->getObj('id = '.$id);
	}
	...
	public function market_add()
	{
		...
		$province    = IFilter::act(IReq::get('province'),'int');
		$city        = IFilter::act(IReq::get('city'),'int');
		$district        = IFilter::act(IReq::get('district'),'int');
		...
		if($sellerDB->getObj("name = '{$name}' and id != {$seller_id}"))
		{
			$errorMsg = "市场全程重复";
		}
		...
		{
			$this->sellerRow = $_POST;
			$this->redirect('market_edit',false);
			Util::showMessage($errorMsg);
		}
		...
		$this->redirect('market_list');
	}

	// 获取当前市场经纬度
	public function getmarket_jw()
	{
		$city    = IFilter::act(IReq::get('city'));
		$address = IFilter::act(IReq::get('address'));
		$res     = area::getTraceByName($city,$address);
		if($res)
		{
			$result = array('code' => 1,'msg' => '获取成功','data'=>$res);
		}else{
			$result = array('code' => 0,'msg' => '经纬度获取失败');
		}
	...
	{
		$id = IFilter::act(IReq::get('id'),'int');
		$sellerDB        = new IModel('freight');
		$sellerRow = $sellerDB->getObj('id = '.$id);
		if($sellerRow)
		{
			$sellerRow['area']=$sellerRow['district'];
			$this->freightRow=$sellerRow;
	...
	//商户状态ajax
	public function ajax_freight_lock()
	{
		$id   = IFilter::act(IReq::get('id'));
		$lock = IFilter::act(IReq::get('lock'));
		$sellerObj = new IModel('freight');
		$sellerObj->setData(array('status' => $lock));
		$sellerObj->update("id = ".$id);
	}
}