...
class goods_class {
	...
	/**
	 * @brief  根据分类ID获取其全部子分类数据(自上向下的获取数据)
	 *
	 ...
	 * @return
	 */
	public static function topcatChildTree($catId, $seller_id = 0)
	{
		$result        = array();
		$catDB         = new IQuery('category');
		$catDB->fields = 'id';
		...
		while (TRUE)
		{
			if ($catRow)
			{
				$a            = array_column($catRow, 'id');
				$str          = join(",", $a);
				$result       = array_merge($result, $a);
				$catDB->where = "parent_id = '{$str}'";
				$catRow       = $catDB->find();
			}
			else
			{
	...
	public static function imgPath($img, $type, $flg = 1)
	{
		// echo $img.'---'.$type.'----'.$flg;
		if ($img == '')
		{
	...
	public function update($id, $postData)
	{
		...
		$cateStr        = join(',', $postData['_goods_category']);
		$cateDB         = new IQuery('category');
		$cateDB->fields = 'count(*) as num';
		$cateDB->where  = 'parent_id in ('.$cateStr.') ';
		$catedata       = $cateDB->find();
		if ($catedata[0]['num'] > 0)
		{
			die("当前分类下存在下级分类，请选择最后一级分类");
		}
		...
		if (isset($postData['_spec_array']))
		{
			...
			foreach ($postData['_spec_array'] as $key => $val)
			{
				foreach ($val as $v)
				{
					...
					if ( ! in_array(array($tempSpec['tip'] => $tempSpec['value']),
						$goods_spec_array[$tempSpec['id']]['value']))
					{
		...
		//赋值goods表默认数据
		$goodsUpdateData['goods_no']     = isset($postData['_goods_no'][$defaultKey])
			? $postData['_goods_no'][$defaultKey] : '';
		$goodsUpdateData['market_price'] = isset($postData['_market_price'][$defaultKey])
			? $postData['_market_price'][$defaultKey] : 0;
		$goodsUpdateData['sell_price']   = isset($postData['_sell_price'][$defaultKey])
			? $postData['_sell_price'][$defaultKey] : 0;
		$goodsUpdateData['cost_price']   = isset($postData['_cost_price'][$defaultKey])
			? $postData['_cost_price'][$defaultKey] : 0;
		$goodsUpdateData['weight']       = isset($postData['_weight'][$defaultKey]) ? $postData['_weight'][$defaultKey]
			: 0;
		...
		if (isset($postData['_spec_array']))
		{
			...
			foreach ($postData['_goods_no'] as $key => $rs)
			{
				$productsData = array(
					...
					'spec_array'   => "[".join(',',
							IFilter::addSlash($this->specArraySort($postData['_spec_array'][$key])))."]",
				);
		...
		if (isset($search['adv_search']) && 1 == $search['adv_search'])
		{
			...
			if (isset($search['create_time']) && ! empty($search['create_time']))
			{
				...
				if ($is_check_0 && $is_check_1)
				{
					...
					{
						$where[] = "go.create_time between '".$create_time[0]." 00:00:00' and '".$create_time[0]
							." 23:59:59'";
					}
					else
					{
						...
						if (0 < $difference)
						{
							$where[] = "go.create_time between '".$create_time[1]." 00:00:00' and '".$create_time[0]
								." 23:59:59'";
						}
						else
						{
							$where[] = "go.create_time between '".$create_time[0]." 00:00:00' and '".$create_time[1]
								." 23:59:59'";
						}
					}
				}
	...
	{
		$data = $product_id ? Api::run('getProductInfo', array('#id#', $product_id))
			: Api::run('getGoodsInfo', array('#id#', $goods_id));
