...
{
	//当前市场订单自动拒单
	public static function auto_refund_order()
	{
		$omDB = new IQuery('order_market');
		$ommDB=new IModel('order_market');

		$oDB=new IQuery('order');
		$oModel=new IModel('order');
		$ogDB=new IQuery('order_goods');
		$refundsDB = new IModel('refundment_doc');

		$time   = date('Y-m-d H:i:s', time() - 60 * 5);
		$omDB->where="pay_time < '$time' and pay_status=1 and status in(2,8)";
		$result=$omDB->find();
		...
		if($result)
		{
			foreach ($result as $k => $v) {
				...
				foreach ($oidArr as $key => $val) {
					...
					{
						// 订单设置为拒单
						$oModel->setData(array('status'=>12));
						$oModel->update('id='.$val);

						$ogDB->fields='id';
						$ogDB->where='order_id='.$val;
						$ogData=$ogDB->find();

						$order_goods_id=join(',',array_column($ogData,'id'));
				...
				// 对当前市场状态进行判断 并设置状态
				self::updateOrderMStatus($oidstr,$v['id']);
	 ...
	 * 获取指定订单运费指定商品运费
	 */
    public static function getorderIdYunfei($oid,$juli='',$ogid='')
	{
		// echo $oid.'<br>';
		// echo $juli.'<br>';
		$omDB = new IModel('order');
		//如果不知道距离获取距离
		if(empty($juli))
		{
			$oRow=$omDB->getObj('id = "'.$oid.'"');	
			$juli=$oRow['juli'];
		}
		$where="`og`.`order_id` IN ('".$oid."')";
		if(!empty($ogid))
		{
			$where.=" and og.id in ('".$ogid."')";
		}
		...
		// echo '<pre>';var_dump($ogData);echo $ogDB->getsql().'<br>';
		$weight=$ogData[0]['weight']/1000;

		//获取变量
		$config=new config('site_config');
		$yunfeiChange=$config->freight_rate_value;
		// echo $yunfeiChange.'<br>';
		
		$yunfei=round(round($yunfeiChange*$juli,2)*$weight,2);

		return $yunfei;
	}
	// 退货状态
	public static function orderDocStatus($oid,$stype='status')
	{
		$model = new IModel('returnment_doc');
		$Row=$model->getObj('order_id = "'.$oid.'"');
		if($Row)
		{
			if($Row['status']==0)
			{
	 ...
	 * @param $order_id 订单ID
	 */
	public static function addGoodsCommentChange($order_id,$omid=0)
	{
		// echo $order_id;
		//获取订单对象
		$orderDB  = new IModel('order');
		$orderRow = $orderDB->getObj('id = '.$order_id);
		// echo '<pre>';var_dump($orderRow);
	...
	public static function getTime($type)
	{
		...
		if($type==1)
		{
			$time=ITime::getAppointTime('+3 hours');
		}else if($type==2){
			$time=ITime::getAppointTime('+6 hours');
		}else if($type==3)
		{
			$time=ITime::getAppointTime('+1 day',2);
			$y=date('Y',$time);
			$m=date('m',$time);
			$d=date('d',$time);
			$h=date('H',$time);
	...
	public static function updateOrderStatus($orderNo,$admin_id = '',$note = '')
	{
		...
		{
			$songda_time=self::getTime($orderRow['status_type']);
			$dataArray = array(
			...
			// 送达时间进行处理
			$dataArray['songda_time']=$songda_time;
			...
			$market_id=$orderRow['market_id'];
			$order_id=$orderRow['id'];
			$uid=$orderRow['user_id'];

			// 1、查询
			$omObj  = new IQuery('order_market');
			$omObj->where='market_id = "'.$market_id.'" and uid="'.$uid.'" and pay_status=0 and find_in_set(\''.$order_id.'\',orderid_str)';
			// echo $omObj->getSql();exit();
			$omData = $omObj->find();
			if($omData[0])
			{
				// 2、修改状态
				$omObj2  = new IModel('order_market');
				$omObj2->setData(array('pay_status' => 1,'status'=>2,'songda_time'=>$songda_time,'pay_time'=>ITime::getDateTime()));
				$omObj2->update('id='.$omData[0]['id']);
			}
			...
			// 获取运费
			$yunfei=self::getorderIdYunfei($orderRow['id'],$orderRow['juli']);
			// echo $yunfei.'<br>';
			// echo '<pre>';var_dump($orderRow);
			...
			$collectionData   = array(
				'order_id'   => $orderRow['id'],
				'user_id'    => $orderRow['user_id'],
				'amount'     => $orderRow['order_amount']+$yunfei,
				'time'       => ITime::getDateTime(),
	...
	public function getOrderMarketShow($omid,$user_id = 0,$seller_id = 0)
	{
		$where = 'id = '.$omid;
		if($user_id !== 0)
		{
		...
		$data = array();
		$tb_order = new IModel('order_market');
 		$data = $tb_order->getObj($where);
 		if($data)
 		{
 			$orderid_Arr=explode(',',$data['orderid_str']);
 			foreach ($orderid_Arr as $key => $value) {
	 			$data['orderlist'][]=self::getOrderShow($value,$user_id);
	 		}
 		}
		...
		$tb_order = new IModel('order');
 		...
 		if($data)
 		{
 			...
 			if($data['seller_id']==0)
 			{
		 		...
		 		$sellerDB->fields='true_name,mobile';
		 		$sellerDB->where='id='.$data['seller_id'];
		 		$sellerD=$sellerDB->find();
		 		// echo $sellerDB->getSql();exit();
		 		$data['sellername']=$sellerD[0]['true_name'];
		 		$data['sellermobile']=$sellerD[0]['mobile'];
	 		}
	    	...
	    	if($delivery_info)
	    	{
	 		...
	 		$config = new Config('site_config');
			$freight_rate_value = $config->freight_rate_value;
	 		// echo $data['goods_weight'].'<br>';
	 		// echo $freight_rate_value.'<br>';
	 		// 对运费进行处理
	 		$data['yunfei']=round(round($data['juli']*$freight_rate_value,2)*($data['goods_weight']/1000),2);
	 		// echo '<br>'.$data['yunfei'];
	 		// exit();
	...
	public function insertOrderGoods($order_id,$goodsResult = array())
	{
		...
		if(isset($goodsResult['goodsList']))
		{
			foreach($goodsResult['goodsList'] as $key => $val)
			{
				...
				$goodsArray['img_pathtype']         = $val['img_pathtype'];
	...
	//获取市场订单配送状态
	public static function getOMFreightStatusText($orderRow)
	{

		if($orderRow['status'] == 11)
		{
			return '餐厅确认送达';
		}
		else if($orderRow['status'] == 12)
		...
		else if($orderRow['freight_status'] == 0)
		{
			return '未抢单';
		}
		else if($orderRow['freight_status'] == 1)
		{
			return '已接单';
		}
		else if($orderRow['freight_status'] == 2)
		{
			return '已取货';
		}
		else if($orderRow['freight_status'] == 3)
		{
			return '已送达';
		}
	}
	...
	public static function isGysDeliver($omstatus,$createtime)
	{
		// echo $omstatus.'----'.$createtime.'<br>';
		$newtime=time();
		$createtime=strtotime($createtime);
		// echo $newtime.'----'.$createtime;
		if(($new-$createtime)>(2*60*60) && $omstatus==8)
		{
			return true;
		}else{
			return false;
	...
	public static function getOMSearchCondition($search)
	{
		// echo '<pre>';var_dump($search);exit();
		$join="left join market as m on m.id = om.market_id";
		$where = "om.if_del = 0";
		if(isset($search['pay_status']) && $search['pay_status'] !== '')
		{
			$pay_status = IFilter::act($search['pay_status'], 'int');
			$where .= " and om.pay_status = ".$pay_status;
		}
		if(isset($search['freight_status']) && $search['freight_status'] !== '')
		{
			$distribution_status = IFilter::act($search['freight_status'], 'int');
			$where .= " and om.freight_status = ".$distribution_status;
		}
		if(isset($search['status']) && $search['status'] !== '')
		{
			$status = IFilter::act($search['status'], 'int');
			$where .= " and om.status = ".$status;
		}
	...
	public static function isRefundmentApply($orderRow,$orderGoodsIds = array())
	{
		...
		{
			//已经付款,并且未全部退款，未正在发货中
			// echo $orderRow['pay_status'].'-----'.$orderRow['status'].'-----'.self::getOrderStatus($orderRow).'<br>';
	...
	public static function returnmentText($pay_status)
	{
		$result = array('0' => '申请退货退款', '1' => '退货退款失败', '2' => '退货退款成功');
		return isset($result[$pay_status]) ? $result[$pay_status] : '';
	}
	...
	public static function updateOrderMStatus($orderidstr,$omid)
	{
		// echo $orderidstr.'<br>';
		$orderQuery=new IQuery('order');
        $orderQuery->fields='status,id';
        $orderQuery->where='id in ('.$orderidstr.') and status in (3,4,8,12)';
        $resultData=$orderQuery->find();

        // echo '<pre>';var_dump($resultData);
        $result=array();
        $a = array_column($resultData, 'status');
		$result=array_merge_recursive($result,$a);
		// echo '<pre>';var_dump($result);

		$count=count(explode(',',$orderidstr));
		if($count==count($resultData))
		{
			$omDB=new IModel('order_market');
			$data=array();
			if(in_array(8,$result))//商户确定
			{
				$data['status']=9;
				$data['seller_finish_time']=date('Y-m-d H:i:s');
			}else if(in_array(12,$result))//商户取消
			{
				//餐厅端取消
				$data['status']=3;
			}else if(in_array(4,$result))
			{
				$data['status']=3;
			}else if(in_array(3,$result))
			{
				$data['status']=3;
			}
			if($data)
	...
	public static function refund($refundId,$authorId,$type = 'admin',$way = 'balance')
	{
		...
		if($orderStatus == 6)
		{
			...
			if($refundsRow['amount'] == 0)
			{
				...
				if($isDeliveryData)
				{
					$amount -= $orderRow['real_freight'];
				}

				//检查之前已经退款的订单
				$hasRefundData = $refundDB->query("order_id = ".$refundsRow['order_id']." and pay_status = 2 and id != ".$refundId,"amount");
				if($hasRefundData)
				{
		...
		//最后检验
		if($amount < 0 || $amount > $orderRow['order_amount'])
		{
			$tb_order->rollback();
			return '请手动填写要退款的金额';
		}
